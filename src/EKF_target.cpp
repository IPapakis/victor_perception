...
Matrix<double, 5, 1> robot_move_matrix;
...
void noise(){
    ...
    sensor_noise= sensor_noise_original*pow((-0.00387020086662241*pow(sqrt(q),3)+0.100960507080783*pow(sqrt(q),2)-0.526807007313129*sqrt(q)+1.41951575268594),2); 
    sensor_noise(1,1)=0.5*0.5;
}
//Process with SLAM
void SLAM(){
    state(0,0)=rx;
    ...
    //now deal with the new landmarks in the current list
        if (target_include==true && stop==false && target_first_time==false){  
            int j=0;
            ROS_INFO_STREAM("size \n"<<lm_current_msg.data.size());
            //resize
            stop=true;
            ...
            noise();
                ...
                covariance.bottomRightCorner(2,2)= M1*covariance.block(0,0,3,3)*M1.transpose()+M2*sensor_noise*M2.transpose(); 
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        //output topic
        output.data.clear();
        //output.data.push_back(sqrt(pow(robot_x,2)+pow(robot_y,2)));// robot estimated distance travelled 
        ...
        output.data.push_back(0.5*(8.51363 + log(covariance.block(0,0,3,3).determinant())));//entropy of robot
        output.data.push_back(log(sqrt(pow(3.14*2*2.71,2)*target_covariance)));// entropy of target
        output.data.push_back(map_measure); // map inf gain
        output.data.push_back(target_measure);// target inf gain
        output.data.push_back(log(1/((M2-K*H2).block(0,0,3,3)).determinant())); //final measure
        output.data.push_back(log(1/((M2-K*H2).block(3,3,2,2)).determinant())); //final measure
        output.data.push_back(log(1/((M2-K*H2)).determinant())); //final measure
        output.data.push_back(robot_move); //final measure
        output.data.push_back(robot_bearing); //final measure

        if (target_include==true){
            output.data.push_back(0.5*(8.51363 + log(covariance.bottomLeftCorner(2,2).determinant()))); //final measure
        }
        //publish odometry
